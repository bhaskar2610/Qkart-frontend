...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      const response = await axios.post(URL, { "address": newAddress, }, {
        headers: {
          Authorization: `Bearer ${token}`,
        }
      });
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if (validateRequest(items, addresses)) {
      ...
      try {
        const response = await axios.post(URL, postAddress, {
          headers: {
            Authorization: `Bearer ${token}`,
          }
        });
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length
                ? addresses.all.map((item) => (
                  <Box key={item._id} className={togleStyle(item._id)} onClick={()=>(setAddresses({...addresses,selected:item._id}))}>
                    <p>{item.address}</p>
            ...
            {showComponent ? <AddNewAddressView
              token={token}
